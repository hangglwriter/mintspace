...
- [x] **작업 복원** (2026-06-13) - 상단바 `[🕐 작업 복원]` → `logs/*.md` 의 cldp launch 기록(`- HH:MM  💬 [pid] cldp`)을 정규식(`_LAUNCH_RE`) 파싱 → **마지막 launch 시각부터 6시간 윈도우 안에 연 프로젝트**(중복 제거, 최근순)를 후보로 → 체크 모달 → **새 창 하나에 탭으로 복원**. launch 코어는 그룹과 공유(`_open_projects_in_one_window`). 폴더 없는 후보는 disabled 흐림. 동기: 터미널 창 전체가 닫혔을 때 일일이 안 열고 한 번에. 개별 cldp(💬)만 추적(그룹 🗂 로그는 멤버 id 없어 제외)
...
- **작업 복원**: GET `/api/restore-candidates?window=360` (마지막 세션 cldp 프로젝트) · POST `/api/restore-launch` (선택 id 들 한 창에 탭으로)
...
- **캐시 버스팅**: `web/index.html` 의 `?v=YYYYMMDDNN` 버전 안 올리면 브라우저가 옛 js/css 사용 (현재 `?v=2026061304`)
